package dev.zgrv.binaryUtils.scheduler

import dev.zgrv.binaryUtils.BinaryUtils
import dev.zgrv.binaryUtils.time.TimeUnit
...
fun sync(runnable: BukkitRunnable.() -> Unit): BukkitTask {
    return createBukkitRunnable(runnable).runTask(BinaryUtils.plugin)
}
...
fun async(runnable: BukkitRunnable.() -> Unit): BukkitTask {
    return createBukkitRunnable(runnable).runTaskAsynchronously(BinaryUtils.plugin)
}
...
fun delay(
    ...
    return if (async) {
        createBukkitRunnable(runnable).runTaskLaterAsynchronously(BinaryUtils.plugin, unit.toTicks(value.toLong()))
    } else {
        createBukkitRunnable(runnable).runTaskLater(BinaryUtils.plugin, unit.toTicks(value.toLong()))
    }
}
